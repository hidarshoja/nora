...
export default function OfferSection() {
  ...
  const calculateOffer = (price,discount) => {
                  ...
                  <SwiperSlide key={index}>
                    <div className="bg-white rounded-3xl leading-10 p-4">
                      <div className="relative">
                        <Link to={`/shop/${product?.product.slug}`} className="flex flex-col items-center justify-center">
                        ...
                        <div className="bg-yellow-500 absolute top-2 right-2 rounded-full w-10 h-10">
                          <p className="flex items-center justify-center">{calculateOffer(Number(product.product.price),Number(product.product.price_with_off))}%</p>
                        </div>
                      </div>
                      <div className="text-center">
                        <Link to={`/shop/${product?.product.slug}`}><h3 className="font-YekanBakh-ExtraBold text-base">{product.product.name}</h3></Link>
