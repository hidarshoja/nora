...
export default function CategoryTable() {
  ...
  const [searchTerm, setSearchTerm] = useState(""); // State برای ذخیره عبارت جستجو

  const { data: category, isLoading, refetch } = useGet(['category'], '/category');
  const { mutateAsync, isPending } = useDelete('/category', ['category']);

  // فیلتر کردن داده‌ها بر اساس نام دسته‌بندی
  const filteredCategories = useMemo(() => {
    if (!category?.data) return [];
    return category.data.filter((cat) =>
              ...
              >
                ...
                <td className="flex items-center justify-center w-full">
                  <img
                    src={
                      category.image_url
                        ? `${import.meta.env.VITE_API_BASE_URL}${category.image_url}`
                        : ""
                    }
                    className="w-10 h-10"
                    alt=""
